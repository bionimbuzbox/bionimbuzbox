...
public class KeyUsageExtension extends CertExtension {
  ...
  KeyUsageExtension(final int keyUsages, final boolean critical) {
    super(Extension.keyUsage, critical, new org.bouncycastle.asn1.x509.KeyUsage(keyUsages));
  }

  KeyUsageExtension(final boolean critical, final KeyUsage... usages) {
    this(getUsages(usages), critical);
  }

  public static KeyUsageExtension create(final KeyUsage... usages) {
    return new KeyUsageExtension(false, usages);
  }
  
  public static KeyUsageExtension create(final boolean critical, final KeyUsage... usages) {
    return new KeyUsageExtension(critical, usages);
  }
